...
# Type: New Service
#
# Title: AZURE - Create Azure Resource Group
#
# v 1.0 Initial Release
#
#
# Init Release: 10.03.2022
# Last Update: 10.03.2022
# Code Template V 1.4
#
# URL: 
# Github: https://github.com/au2mator/AZURE-Remove-Resource-Group
#################
...
param (
    ...
    [String]$c_Subscription,

    [parameter(Mandatory = $true)]
    [String]$c_ResourceGroup,
...
[string]$LogPath = "C:\_SCOworkingDir\TFS\PS-Services\AZURE - Delete Azure Resource Group"
[string]$LogfileName = "AZURE - Delete Azure Resource Group"
...
try {
    ...
    try {
        Write-au2matorLog -Type INFO -Text "Try to remove Azure Resource Group"

        $URL = "https://management.azure.com/subscriptions/$c_Subscription/resourceGroups/$($c_ResourceGroup)?api-version=$apiversion"
        Invoke-RestMethod -Method DELETE -URI $URL -headers $headers 
    
        $Status = "COMPLETED"
    }
    catch {
        Write-au2matorLog -Type ERROR -Text "Error to create Resource Group"
        Write-au2matorLog -Type ERROR -Text $Error
    
        $au2matorReturn = "Error to create Resource Group, Error: $Error"
        $TeamsReturn= "Error to create Resource Group" #No Special Characters allowed
        $AdditionalHTML = "Error to create Resource Group
        <br>
        Error: $Error
            "
        $Status = "ERROR"
